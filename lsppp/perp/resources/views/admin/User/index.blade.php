        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6 flex flex-col md:flex-row justify-between items-center gap-4 border-b border-gray-100">
                ...
                <div class="flex items-center gap-3 w-full md:w-auto justify-end">
                    <!-- Add Button -->
                    <a href="/users/create" class="bg-[#fbbf24] hover:bg-yellow-500 text-gray-900 px-5 py-2.5 rounded-lg text-sm font-semibold flex items-center gap-2 transition-colors shadow-sm">
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="text-gray-600 text-sm">
                        ...
                        <tr class="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                            ...
                            <td class="p-4 font-medium text-gray-700">{{ $user->username }}</td>
                            <td class="p-4">{{ $user->anggota?->nama ?? '-' }}</td>
                            <td class="p-4">{{ $user->anggota?->nis ?? '-' }}</td>
                            <td class="p-4">
                                @if($user->anggota?->jurusan)
                                    <span class="px-2 py-1 bg-blue-50 text-blue-700 rounded text-xs font-medium">{{ $user->anggota->jurusan }}</span>
                                @else
                                    -
                                @endif
                            </td>
                            <td class="p-4">{{ $user->anggota?->kelas ?? '-' }}</td>
                            
                            <td class="p-4 text-center">
                                <div class="flex items-center justify-center gap-3">
                                    
                                    <!-- Edit -->
                                    <a href="{{ url('/users/' . $user->id . '/edit') }}" class="text-gray-400 hover:text-yellow-600 transition-colors" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                    </a>
                                    <!-- Delete -->
                                    <form action="/users/{{ $user->id }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this user?');" class="inline">
                                        @csrf
                                        @method('DELETE')
            ...
            <div class="p-4 border-t border-gray-100 flex flex-col md:flex-row items-center justify-between text-sm text-gray-500 gap-4">
                ...
                <div class="flex items-center gap-4">
                    <span class="text-gray-600">Showing <span class="font-medium text-gray-900">1-{{ count($users) }}</span> of <span class="font-medium text-gray-900">{{ count($users)}}</span> records</span>
