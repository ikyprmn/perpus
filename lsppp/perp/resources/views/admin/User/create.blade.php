...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-6">
            ...
            <p class="text-slate-500 text-sm mt-1">Create a new user account with specific role and permissions.</p>
        </div>

        <!-- Form Card -->
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            <form action="{{ url('users/create') }}" method="POST" class="p-6 space-y-8">
                @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-1">
                            ...
                            <select name="role" id="role" class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-lg focus:border-slate-800 focus:ring-1 focus:ring-slate-800 outline-none transition-all text-slate-800" onchange="toggleAnggotaFields()">
                                <option value="siswa" {{ old('role') == 'siswa' ? 'selected' : '' }}>Member (Siswa)</option>
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="text" name="username" id="username" value="{{ old('username') }}" placeholder="username123" class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-lg focus:border-slate-800 focus:ring-1 focus:ring-slate-800 outline-none transition-all placeholder:text-gray-400 text-slate-800" required>
                        </div>

                        <!-- Password -->
                        <div class="space-y-1">
                            <label for="password" class="block text-sm font-medium text-slate-700">Password</label>
                            <input type="password" name="password" id="password" placeholder="••••••••" class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-lg focus:border-slate-800 focus:ring-1 focus:ring-slate-800 outline-none transition-all placeholder:text-gray-400 text-slate-800" required>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-1">
                            ...
                            <select name="jurusan" id="jurusan" class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-lg focus:border-slate-800 focus:ring-1 focus:ring-slate-800 outline-none transition-all text-slate-800 disabled:opacity-50 disabled:bg-gray-100" required>
                                ...
                                <option value="TR" {{ old('jurusan') == 'TR' ? 'selected' : '' }}>Teknik Transmisi Telekomunikasi (TR)</option>
                            </select>
                        </div>
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="text" name="kelas" id="kelas" value="{{ old('kelas') }}" placeholder="e.g. XII Tel 13" class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-lg focus:border-slate-800 focus:ring-1 focus:ring-slate-800 outline-none transition-all placeholder:text-gray-400 text-slate-800 disabled:opacity-50 disabled:bg-gray-100" required>
                        </div>
                    </div>
                    <p class="text-xs text-slate-500 mt-2 italic">* These fields are required for Member role.</p>
                </div>
                ...
                <div class="pt-4 flex items-center justify-end gap-3 border-t border-gray-50">
                    ...
                    <button type="submit" class="px-6 py-2 text-sm font-medium text-white bg-slate-900 rounded-lg hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-900 shadow-sm shadow-slate-900/10 transition-all">Create User</button>
                </div>
            </form>
        </div>
    </div>
        ...
        function toggleAnggotaFields() {
            ...
            if (role === 'admin') {
                ...
                inputs.forEach(input => {
                    ...
                    // Keep values if switching back and forth, or clear them?
                    // Let's keep them visual but disabled handles submission
                });
            } else {
